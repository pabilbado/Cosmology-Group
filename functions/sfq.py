...
#           Outputs: the result of the function from given values.
    def cal(self, a):
        w = -1 #(-1 + (self.wpas - self.wf)/(1 + math.pow(a/self.at, 1./self.tau))) #function for the barotropic parameter in terms of a
...
class intdsfq(Function):
    ...
    def cal(self, a):   #calculates the integral for d(z). The integrand is long, and is therefore
        ...
        sqrt_part = math.sqrt(self.omeM*math.pow(self.a0/a, 3) + self.omeX*self.rho.cal(self.z))
